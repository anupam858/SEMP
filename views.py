...
def signup_c(request):
      ...
      if request.method=='POST':
            ...
            email1 = request.POST.get('cemail')
            phone1 = request.POST.get('cnumber')
            password1 = request.POST.get('cpswd')
            #password1 = make_password(password, salt=None, hasher='default')
            ...
            user = User.objects.create_user(name1, email1, password1)
            user.save()
            
      
      return(render(request,'home\customer1.html',{'message':'User Registered'}))

def chome(request):

      print('This view is being rendered')

      return(render(request, 'home\customer1.html', {}))
